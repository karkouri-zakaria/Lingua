...
class AppSidebar:
    ...
    def download_answers(self):
        with expander("Download", expanded=False, icon="📥"):
            ...
            download_button("💹 Mistakes.xlsx", buffer.getvalue(), f"Mistakes_{session_state.uploaded_file_data.name.split('.')[0]}.xlsx", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", use_container_width=True)
            download_button("🗃️ Answers.json", dumps([[d[0], d[1], d[2], d[3]] for d in session_state.Answers], ensure_ascii=False, indent=4), f"Answers_{session_state.uploaded_file_data.name.split('.')[0]}.json", use_container_width=True)
