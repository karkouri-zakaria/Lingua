...
def main():
    ...
    if session_state.flashcards_df is not None:
        ...
        if len(session_state.Results) == total_flashcards and total_flashcards != 0 and not session_state.show_wrongs:
            ...
            quiz_tab, all_cards = tabs(["🎮 **Quiz**", f"📓 **All cards of {session_state.uploaded_file_data.name if not session_state.flashcards_df is None else "All cards"}**"])
            with quiz_tab:
                ...
                with sidebar:
                    if session_state.flashcards_df is not None: 
                        ...
                        left_button, right_button = sidebar.columns([3, 1], gap="small" , vertical_alignment="bottom")
                        if len(session_state.Results) > 1 or not session_state.show_wrongs:
                            try:
                                page_input = left_button.number_input(f"Card (Total = {total_flashcards}) :", min_value=1, max_value=len(session_state.flashcards_df), step=1, value=session_state.flashcard_index+1, key="page_input")
                            except:
                                page_input=0
